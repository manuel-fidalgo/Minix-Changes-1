 ...
 */
  ...
  int front;
  struct proc *aux_1, *aux_2;			/* add to front or back */
  ...
  else if (prioridad_procesos[rp->p_nr + NR_TASKS] < prioridad_procesos[rdy_head[q]->p_nr + NR_TASKS]){	/* add to head of queue */
      rp->p_nextready = rdy_head[q];		/* chain head of queue */
      rdy_head[q] = rp;				/* set new queue head */
  } 
  else if(prioridad_procesos[rp->p_nr + NR_TASKS] > prioridad_procesos[rdy_tail[q]->p_nr+ NR_TASKS]) {	/* add to tail of queue */
      rdy_tail[q]->p_nextready = rp;		/* chain tail of queue */	
      rdy_tail[q] = rp;				/* set new queue tail */
      rp->p_nextready = NIL_PROC;		/* mark new end */
  }else {
  	aux_1 = rdy_head[q];
  	aux_2 = rdy_head[q]->p_nextready;
  	while(aux_2->p_nr < rp->p_nr){
  		aux_1 = aux_1->p_nextready;
  		aux_2 = aux_2->p_nextready;
  	}
  	aux_1->p_nextready = rp;
 	rp->p_nextready = aux_2;
  }
